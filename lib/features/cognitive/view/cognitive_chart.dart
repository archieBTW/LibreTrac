...
class _CognitiveChartCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    switch (kind) {
      case CogTestKind.reaction:
        return ref
            .watch(reactionResultsStreamProvider)
            .when(
      ...
      case CogTestKind.stroop:
        return ref
            .watch(stroopResultsStreamProvider)
            .when(
              loading: () => _buildCard(context, [], kind),
              error: (e, _) => _buildCard(context, [], kind),
              data:
      ...
      case CogTestKind.goNoGo:
        return ref
            .watch(goNoGoResultsStreamProvider)
            .when(
              loading: () => _buildCard(context, [], kind),
              error: (e, _) => _buildCard(context, [], kind),
              data:
      ...
      case CogTestKind.digitSpan:
        return ref
            .watch(digitSpanResultsStreamProvider)
            .when(
              loading: () => _buildCard(context, [], kind),
              error: (e, _) => _buildCard(context, [], kind),
              data:
      ...
      case CogTestKind.symbolSearch:
        return ref
            .watch(symbolSearchResultsStreamProvider)
            .when(
              loading: () => _buildCard(context, [], kind),
              error: (e, _) => _buildCard(context, [], kind),
              data:
  ...
  List<T> _processRows<T>(
    ...
    var rows =
        allRows.where((r) => getTimestamp(r).isAfter(cutoff)).toList()
  ...
  Widget _buildCard(
    ...
    double toPct(double v) {
      ...
      final frac =
          kind.lowerIsBetter ? (hi - clamped) / span : (clamped - lo) / span;
      return frac * 100;
    }
